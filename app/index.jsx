import Component from "../react/component/index.js";
// import { h } from "start-dom-jsx";
import { createElement as h } from "../react/jsx";
import { subscribe, useEffect, useState } from "../react";
...
class Number extends Component {
	...
	render() {
		return <div className="alert-container">{this.props.number}</div>;
	}
}
// Stateful component
class Alert extends Component {
	constructor(props) {
		...
		this.changeNumber = this.changeNumber.bind(this); // Binding the context to instance of Alert
	}
	...
	changeTitle() {
		this.setState({ ...this.state, title: new Date().toString() });
	}
	...
	render() {
		return (
			<div className="alert-container">
				...
				<Button onClick={this.changeNumber}>{this.state.number}</Button>
				{this.state.number > 2 && this.state.number < 6 && (
		...
		return (
			<div>
				...
				<input
					type="text"
					ref={(inputWish) => {
						this.inputWish = inputWish;
					}}
					placeholder="What's your wish?"
				></input>
				<button onClick={this.update}>Update</button>

				<div>{this.state.wish.title}</div>
			</div>
		);
	}
}
