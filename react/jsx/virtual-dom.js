...
export function createElement(type, attributes = {}, ...children) {
	...
	let component;
	const props = Object.assign({ children: childElements }, attributes);
	console.log(
		Object.getPrototypeOf(type)?.prototype?.isReactComponent,
		"proto"
	);
	if (
		!Object.getPrototypeOf(type)?.prototype?.isReactComponent &&
		...
		// component.prototype.props = Object.assign(
		// 	{ children: childElements },
		// 	attributes
		// );
		temp.prototype.render = type;
		// temp.prototype.render;
		temp.prototype.isFunctionalComponent = true;

		component = Object.freeze(temp);
...
export const diff = function (vdom, container, oldDom) {
...
function updateComponent(
	...
	// console.log(oldComponent.shouldComponentUpdate(newVirtualElement.props));
	// Lifecycle method
	if (oldComponent.shouldComponentUpdate(newVirtualElement.props)) {
		...
		// Call Render
		// Generate new vdom
		const nextElement = Object.getPrototypeOf(oldComponent)
			.isFunctionalComponent
			? oldComponent.render(oldComponent.props)
			: oldComponent.render();
...
function buildFunctionalComponent(vnode, context) {
	vnode.sequence = global.genSequence();
	return vnode.type(vnode.props || {});
}

function buildStatefulComponent(virtualElement) {
	const sequence = global.genSequence();
	const component = new virtualElement.type(virtualElement.props);
	component.setSequence(sequence);
	const nextElement = Object.getPrototypeOf(component).isFunctionalComponent
		? component.render(virtualElement.props)
		: component.render();
	nextElement.component = component;
	global.addState(sequence, component.state);
	return nextElement;
}
...
function mountComponent(vdom, container, oldDomElement) {
	...
	let funcComponent = isFunctionalComponent(vdom);
	// console.log(funcComponent, vdom);

	// if (funcComponent) {
	// 	nextvDom = buildFunctionalComponent(vdom);
	// 	current.methods.update = (props, state) => {
	// 		let dom = oldDomElement;
	// 		let container = component;
	// 		// console.log(vdom);

	// 		let newvdom = nextvDom;
	...
	// console.log(component.getSequence(), "sequence");
	global.addMethod(
		"update",
		component.updateComponent,
		component.getSequence()
	);
	// current.methods.update = component.updateComponent;
	// }
